...
def benchmark_onnx_runtime() -> Dict[str, float]:
    ...
    try:
        ...
        model_name = "distilbert-base-uncased-finetuned-sst-2-english"
        AutoTokenizer.from_pretrained(model_name)  # verifies the model/tokenizer download works
...
def benchmark_summary():
    ...
    # NOTE (restoration audit, 2026-08): the "Comparison" block below used
    # to print fixed marketing numbers (~5ms / 45x) regardless of what
    # `results` actually measured - `results` was computed and discarded.
    # Print what was actually measured instead of hardcoded claims.
    logger.info("\n\nComparison (raw results, see README for methodology caveats):")
