...
class DeploymentManager:
    ...
    def promote_canary(self) -> None:
        ...
        # NOTE (restoration audit, 2026-08): CanaryDeployment.promote_canary()
        # moves canary_version into current_version and then clears
        # canary_version to None. Reading self.canary.canary_version here
        # (after that call) always raised AttributeError on None. Read
        # current_version instead, which now holds the promoted version.
        self.canary.promote_canary()
        self.active_deployment = self.canary.current_version.version
        self.canary = None
        logger.info("Promoted canary to production")
