...
class OpenTelemetryBackend(MetricBackend):
    ...
    def log_metric(self, metric: InferenceMetric) -> None:
        ...
        try:
            ...
            # TODO: observable gauges need a callback to actually report
            # values; creating one here without a callback registers it but
            # never feeds it data, so it's not wired up yet.
...
class DatadogBackend(MetricBackend):
    ...
    def log_metric(self, metric: InferenceMetric) -> None:
        ...
        try:
            ...
            api.Metric.send(
                metric="pystreamai.inference.latency",
