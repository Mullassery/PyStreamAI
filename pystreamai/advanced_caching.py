...
class ResultCache:
    ...
    def __init__(self, ttl_seconds: int = 3600):
        ...
        self.cache: Dict[str, CacheEntry] = {}
        # NOTE (restoration audit, 2026-08): invalidate() used to look for
        # `model_id in k` where k is an MD5 hex digest of
        # "{model_id}:{input}:{version}" - the raw model_id is never a
        # substring of its own hash, so invalidation silently never matched
        # anything. Track keys per model_id explicitly instead.
        self._keys_by_model: Dict[str, set] = {}
    ...
    def set(self, model_id: str, input_data: Dict[str, Any], result: Any, version: str = "v1") -> None:
        ...
        self.cache[key] = entry
        self._keys_by_model.setdefault(model_id, set()).add(key)

    def invalidate(self, model_id: str) -> None:
        """Invalidate all results for model (e.g., after retraining)"""
        to_remove = self._keys_by_model.pop(model_id, set())
        for k in to_remove:
