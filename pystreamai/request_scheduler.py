...
class RequestScheduler:
    ...
    def enqueue(self, request: ScheduledRequest) -> bool:
        ...
        # NOTE (restoration audit, 2026-08): _check_user_quota() reads
        # request_counts[f"user:{user_id}"], but nothing ever wrote that key
        # before this fix - user quotas were silently unenforceable. Track
        # the per-user counter alongside the per-model one.
        if request.user_id:
