...
mod tests {
    ...
    fn from_string_is_case_insensitive() {
        assert!(matches!(GPUType::from_string("a100"), GPUType::A100));
        assert!(matches!(GPUType::from_string("A100"), GPUType::A100));
    }

    #[test]
    fn unknown_gpu_name_maps_to_unknown() {
        assert!(matches!(GPUType::from_string("RTX-9999"), GPUType::Unknown));
    }

    #[test]
    fn unknown_gpu_has_zero_capabilities() {
        let gpu = GPUType::Unknown;
        assert_eq!(gpu.tensor_cores(), 0);
        assert_eq!(gpu.memory_gb(), 0);
        assert!(!gpu.supports_tensorrt());
        assert!(!gpu.supports_mixed_precision());
        assert!(!gpu.supports_sparsity());
    }

    #[test]
    fn v100_does_not_support_native_int8() {
        assert_eq!(GPUType::V100.int8_tflops(), 0.0);
    }

    #[test]
    fn only_a100_and_h100_support_sparsity() {
        assert!(GPUType::A100.supports_sparsity());
        assert!(GPUType::H100.supports_sparsity());
        assert!(!GPUType::T4.supports_sparsity());
        assert!(!GPUType::V100.supports_sparsity());
    }

    #[test]
    fn tensorrt_optimizer_speedup_compounds_enabled_features() {
        // L4 supports mixed precision (fp16 auto-enabled by ::new) but not
        // structured sparsity, so this isolates the fp16 * int8 factors.
        let mut opt = TensorRTOptimizer::new(GPUType::L4);
        opt.enable_int8();
        let speedup = opt.expected_speedup();
        assert!((speedup - (1.5 * 2.5)).abs() < 1e-4);
    }

    #[test]
    fn multi_gpu_scheduler_round_robins() {
        let mut scheduler = MultiGPUScheduler::new(2, "A100".to_string());
        let first = scheduler.next_gpu().device_id;
        let second = scheduler.next_gpu().device_id;
        let third = scheduler.next_gpu().device_id;
        assert_eq!(first, 0);
        assert_eq!(second, 1);
        assert_eq!(third, 0);
    }
    ...
    #[test]
    fn multi_gpu_scheduler_total_vram() {
        let scheduler = MultiGPUScheduler::new(4, "A100".to_string());
        assert_eq!(scheduler.total_vram_gb(), 4 * 80);
    }
}
